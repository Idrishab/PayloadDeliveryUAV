clc
clear
VH = 0.3; % horizontal tail volume coefficient
Kc = 1.1; % Fuselage aft shape correction factor
MAC = 0.164; % wing MAC (m)
S = 0.1878; % Wing area (m^2)
Df = 0.1; % Fuselage diameter (m)
lL = 0.6; % Ratio of horizontal tail arm to aircraft legnth
Cm_af = -0.099; % wing airfoil pitching moment
AR = 7; % Wing aspect ratio
swp = 0; % Wing sweep angle (deg)
alfa_twist = 0; % Wing twist angle (deg)
Wavg = 11.772; % Aircraft average weight (N)
Vc = 14.6154; hc = 15; % Cruise speed (m/s) and cruise altitude (m)
nh = 0.9; % Horizontal tail (HT) efficiency
...
y=(L0*y0)+(L1*y1)+(L2*y2);
rho_c=y*1.225 % Air density at cruise altitude (kg/m^3)
