function Prel_design=Prel_design(12,44,324,445)
%rho=air density at sea level, CLmax=maximum lift coefficient
%WL=wing loading, PL=power loading
WL=15:0.1:70; % wing loading (N/m^2)
S = 0.1818; % Wing area (m^2)
Wto=2*9.81; % takeoff weight (N)
Pmax = 10; %power (W)

%STALL SPEED
rho=1.225; CLmax=1.6;
PL_stall=0:0.05:3; % Power loading (N/W)
WLs=rho*(Vs^2)*CLmax/2; % stall wing loading (N/m^2)
%MAXIMUM SPEED
pi=3.142; AR=7; np=0.6;%AR=aspect ratio, np=propeller efficiency
e= (1.78*(1-(0.0045*(AR^0.68))))-0.64; %e=oswald span efficiency factor for straight wing
...
CLc=0.66; delta_CL_flap_to=0.55;%flap lift coefficient at takeoff
